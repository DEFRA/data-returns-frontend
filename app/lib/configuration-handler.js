...
function getYMLFileConfiguration (mode) {
    ...
    for (let i = 0; i < sections.length; i++) {
        const conf = ymal.parse(sections[i]);
    ...
    if (!config[mode] || !config.global) {
        throw new Error(`Cannot find either global or environment section specified by: ${mode} in the configuration file: ${applicationFileName}`);
    }
...
function Configuration () {
    ...
    try {
        ...
        const environmentVariables = getRequiredEnvironmentVariables();
        const yMLFileConfiguration = getYMLFileConfiguration(process.env.NODE_ENV);
        ...
        winston.error(err);
        throw new Error('Cannot load configuration...halting');
    }
}
...
function envVarSubstitution (elem) {
    if (typeof elem === 'string') {
        ...
        const match = pattern.exec(elem);
        if (match) {
            const evaluate = process.env[match[1]];
            if (!evaluate) {
                throw new Error('Not set; required environment variable: ' + match[1]);
            } else {
...
Configuration.prototype.get = function (item) {
    const result = get(this._configurationObject, item);
