...
const persistFileWithErrors = function (sessionId, fileUuid, fileDetails) {
...
const persistFile = function (sessionId, fileDetails) {
...
module.exports = {
    ...
    createFileDetailsJson: function (uuid, clientFilename, errorCode) {
        ...
        if (errorCode) {
            // More details url
            const urlPrefix = errorCode === 900 ? '/correction/table?uuid=' : '/file/invalid?uuid=';
            const moreDetailsUrl = `${urlPrefix}${uuid}`;
    ...
    getFileData: function (request, sessionID) {
        ...
        if (!request.payload) {
            ...
            const uploads = Array.isArray(request.payload.fileUpload) ? request.payload.fileUpload : [request.payload.fileUpload];
    ...
    processUploadedFile: function (fileData) {
        ...
        return new Promise(function (resolve, reject) {
            if (fileData.localFilename === null || fileData.errorCode) {
                const fileDetails = self.createFileDetailsJson(fileData.id, fileData.clientFilename, fileData.errorCode);
                const response = {
                    'success': false,
                    'preventRetry': true,
            ...
            const fileSize = fs.statSync(fileData.localFilename).size;
            ...
            csvValidator.validateFile(fileData.localFilename, fileSize).then(function () {
                ...
                const processingResult = self.createFileDetailsJson(fileData.id, fileData.clientFilename, errorCode);
