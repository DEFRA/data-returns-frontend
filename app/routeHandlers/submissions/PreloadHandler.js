...
function createSession () {
    return new Promise(function (resolve, reject) {
        ...
        const backendData = lodash.merge({'internalKey': uuidGen.v4()}, publicInfo);
    ...
    redisKeys.PRELOADED_SESSIONS.compositeKey(reqInfo.sessionId)
        ...
        .then((sessionData) => {
            if (!sessionData || reqInfo.sessionKey !== sessionData.sessionKey) {
                const reason = !sessionData ? 'No session data' : `Session key mismatch.  Requested: ${reqInfo.sessionKey}, Found: ${sessionData.sessionKey}`;
    ...
    if (request.query.createSession) {
        ...
        redisKeys.PRELOADED_SESSIONS.compositeKey(reqInfo.sessionId)
            ...
            .then((sessionData) => {
                ...
                const fileData = fileUploadProcessor.getFileData(request, sessionData.internalKey);

                winston.info('Preload Handler: Processing uploaded files.');
                let callbacks = 0;
                const result = lodash.merge({}, reqInfo, {
                    'files': []
                });
                const resultHandler = function (processorResult) {
