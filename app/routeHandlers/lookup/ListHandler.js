...
module.exports = {
    ...
    getDisplayHandlerWithSearch: function (request, reply) {
        ...
        if (!searchString || !searchString.length) {
            ...
            resultHandler = function (metadata, headings, data) {
                ...
                const entryText = data.length === 1 ? 'entry' : 'entries';
                const searchTermStrings = searchTerms.map(t => `"${t}"`).join(', ').replace(/,(?!.*,)/gmi, ' or');
                const searchableHeadings = metadata.searchFields.map(t => metadata.displayHeaders.find(d => d.field === t));
    ...
    getCSVHandler: function (request, reply) {
        if (!request.params || !request.params.list) {
            throw new Error('Excepted - filename');
        }
        ...
        handler.getListProcessor(handler.csvExtractorPivot, request.params.list, function (metadata, header, data) {
            const columns = header.map(h => h.description);
